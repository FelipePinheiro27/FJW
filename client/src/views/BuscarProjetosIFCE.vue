<template>
  <div class="img_fundo">
    ...
    <div class="container" id="fundo_IFCE">
      <br> <br>
      <div class="top_cp">
        <br />
        <h2 class="text-center" id="logo_cp">
          Projetos desenvolvidos no Instituto Federal do Ceará
        </h2>
        <br>
      </div>
      <br><br>
    ...
    <table class="table table table-bordered">
        ...
        <tbody id="tbody" v-for="projeto in projects" :key="projeto.id">
           ...
           <tr class="table-light" v-for="user in users" :key="user.id" >
            <td v-if="user.id == projeto.user_id && user.instituicao == 'IFCE'" >{{user.curso}}</td> 
            <td v-if="user.id == projeto.user_id && user.instituicao == 'IFCE'" >{{projeto.tipo}}</td>
            <td v-if="user.id == projeto.user_id && user.instituicao == 'IFCE'" >{{projeto.titulo}}</td>
            <td v-if="user.id == projeto.user_id && user.instituicao == 'IFCE'">{{user.login}}</td>
              <!-- <td v-if="user.id == projeto.user_id">   <div id="img_estrela"></div></td> -->
              <td v-if="user.id == projeto.user_id && user.instituicao == 'IFCE'">
...
#fundo_IFCE{
background-image:linear-gradient( rgba(255,255,255,.8) 0%,rgba(255,255,255,.8) 100%), url("IFCE.jpg"); 
