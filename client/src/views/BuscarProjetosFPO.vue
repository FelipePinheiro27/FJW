<template>
  <div class="img_fundo">
    ...
    <div class="container" id="fundo_FPO">
      <br> <br>
      <div class="top_cp">
        <br />
        <h2 class="text-center" id="logo_cp">
          Projetos desenvolvidos na Faculdade Princesa do Oeste
        </h2>
        <br>
      </div>
      <br><br>
    ...
    <table class="table table table-bordered">
        ...
        <tbody id="tbody" v-for="projeto in projects" :key="projeto.id">
           ...
           <tr class="table-light" v-for="user in users" :key="user.id" >
            <td v-if="user.id == projeto.user_id && user.instituicao == 'FPO'" >{{user.curso}}</td> 
            <td v-if="user.id == projeto.user_id && user.instituicao == 'FPO'" >{{projeto.tipo}}</td>
            <td v-if="user.id == projeto.user_id && user.instituicao == 'FPO'" >{{projeto.titulo}}</td>
            <td v-if="user.id == projeto.user_id && user.instituicao == 'FPO'">{{user.login}}</td>
              <!-- <td v-if="user.id == projeto.user_id">   <div id="img_estrela"></div></td> -->
              <td v-if="user.id == projeto.user_id && user.instituicao == 'FPO'">
                <div v-if="logged == true">
...
#fundo_FPO{
background-image:linear-gradient( rgba(255,255,255,.8) 0%,rgba(255,255,255,.8) 100%), url("FPO.png"); 
